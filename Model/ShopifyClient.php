...
class ShopifyClient  
{
	...
	public function __construct($store_name, $token, $api_key, $secret,$logger) {
			
		$this->name = "ShopifyClient";
		$this->shop_domain = $store_name;		
		$this->logger = $logger;
        $this->secret = $secret;
    ...
    static public function GET_SETTINGS($controller) {		
        $yaml = new Parser();        
        $yaml = $yaml->parse(file_get_contents($controller->get('kernel')->getRootDir().'/config/parameters.yml'));
		$store_name = $controller->get('request')->query->has('shop') ? $controller->get('request')->query->get('shop') : $controller->get('request')->getSession()->get('shop');
		$controller->get('request')->getSession()->set('shop',$store_name);
